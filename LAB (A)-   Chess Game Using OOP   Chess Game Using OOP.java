...
abstract class Piece {
    private booleanisWhite;   // encapsulation: private field
    private booleanhasMoved;

    public Piece(booleanisWhite) {
        this.isWhite = isWhite;
        this.hasMoved = false;
    }

    public booleanisWhite() { returnisWhite; }
    public booleanhasMoved() { returnhasMoved; }
    public void setMoved() { this.hasMoved = true; }

    // Polymorphism: every piece will define canMove in its OWN way
    public abstract booleancanMove(Board board, Spot start, Spot end);
...
class King extends Piece {
    public King(booleanisWhite) { super(isWhite); }

    @Override
    public booleancanMove(Board board, Spot start, Spot end) {
    ...
    @Override
    public char getSymbol() { returnisWhite() ? 'K' : 'k'; }
}

// ===================== QUEEN =====================
class Queen extends Piece {
    public Queen(booleanisWhite) { super(isWhite); }
    ...
    @Override
    public char getSymbol() { returnisWhite() ? 'Q' : 'q'; }
}

// ===================== ROOK =====================
class Rook extends Piece {
    public Rook(booleanisWhite) { super(isWhite); }
    ...
    @Override
    public char getSymbol() { returnisWhite() ? 'R' : 'r'; }
}

// ===================== BISHOP =====================
class Bishop extends Piece {
    public Bishop(booleanisWhite) { super(isWhite); }
    ...
    @Override
    public char getSymbol() { returnisWhite() ? 'B' : 'b'; }
}

// ===================== KNIGHT =====================
class Knight extends Piece {
    public Knight(booleanisWhite) { super(isWhite); }

    @Override
    public booleancanMove(Board board, Spot start, Spot end) {
        int dx = Math.abs(start.getX() - end.getX());
        int dy = Math.abs(start.getY() - end.getY());
        booleanlShape = (dx == 1 &&dy == 2) || (dx == 2 &&dy == 1);   // knight moves in "L" shape
        if (!lShape) return false;
        return Board.destinationOk(this, end);
    }

    @Override
    public char getSymbol() { returnisWhite() ? 'N' : 'n'; }
}

// ===================== PAWN =====================
class Pawn extends Piece {
    public Pawn(booleanisWhite) { super(isWhite); }
    ...
    @Override
    public char getSymbol() { returnisWhite() ? 'P' : 'p'; }
}
...
class Spot {
    ...
    public booleanisEmpty() { return piece == null; }
}
...
class Board {
    ...
    public booleanisPathClear(Spot start, Spot end) {
    ...
    public static booleandestinationOk(Piece mover, Spot end) {
    ...
    public booleanisInCheck(boolean white) {
    ...
    public booleanisCheckmate(boolean white) {
            ...
            for (int x1 = 0; x1 < 8; x1++) {
                ...
                for (int y2 = 0; y2 < 8; y2++)
                    for (int x2 = 0; x2 < 8; x2++) {
                        ...
                        booleanstillInCheck = isInCheck(white);
...
// ===================== PLAYER =====================
class Player {
    private final booleanisWhite;
    public Player(booleanisWhite) { this.isWhite = isWhite; }
    public booleanisWhite() { returnisWhite; }
    public String getName() { returnisWhite ? "White" : "Black"; }
}
...
class Game {
    ...
    public void play() {
        ...
        while (true) {
            ...
            booleanopponentWhite= !current.isWhite();
